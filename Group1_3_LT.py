...
first_day = datetime(2021, 1, 10)
last_day = datetime.today()

data = yf.Ticker(ticker1).history(interval = '2m', start=first_day, end=last_day)
    ...
    try:#this try/except KeyError statement helps skip the loop in which we have negative indexing 
        #(ie. the first loop period is 0 so period-1 is -1 which cannot be used as an index) 
            ...
            if (position_state == 0) and (SMA_signal(data, period, "SMA_Close_50", "SMA_Close_200") == 1):
                ...
                position_state = 1
                Portfolio.Holdings[period] = (Portfolio.Cash[period-1]-comission)//(Portfolio.Price[period] + slippage)
                Portfolio.Cash[period] = Portfolio.Cash[period-1] - Portfolio.Holdings[period] * (Portfolio.Price[period] + slippage) - comission
                Portfolio.Position[period] += Portfolio.Holdings[period] * (Portfolio.Price[period] + slippage)
            ...
            #This elif statement checks if a sell signal is actived and sells if we already bought beforehand (we could short-sell too but it is kept simple since the strategy is not the main goal here) OR if one of the SL/TP levels have been crossed
                ...
                Portfolio.Cash[period] = Portfolio.Cash[period-1] + (Portfolio.Holdings[period-1] * (Portfolio.Price[period] + slippage)) - comission
...
plt.scatter(Portfolio.index[Portfolio.Stoploss!=0],abs(Portfolio.Stoploss[Portfolio.Stoploss!=0]))
plt.scatter(Portfolio.index[Portfolio.Takeprofit!=0],abs(Portfolio.Takeprofit[Portfolio.Takeprofit!=0]))
plt.show()
#"""